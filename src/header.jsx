...
const stripePromise = loadStripe(paykey); 
  ...
  const handlePayment = async () => {
    ...
    return (
        <header>
            <div className="overflow-y-hidden fixed top-0 left-0 z-50 h-0.02vh p-12 md:h-24 xl:h-24 flex flex-col md:flex-row xl:flex-row gap-8 md:gap-24 xl:gap-24 justify-center items-center
             no-underline text-base bg-slate-950 text-slate-950 font-semibold font-(font-family: 'Circular', 'Circular Std', 'Gotham', 'Montserrat', sans-serif)">
                ...
                />
                    ...
                    className="placeholder-white resize-none text-center rounded-full truncate font-medium text-xl border-none w-80 h-10 py-2 transform -translate-x-[6rem]"
                    name="search"
                    id="search"
                    rows={3}
                    cols={50}
